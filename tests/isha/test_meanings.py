...
def test_get_meaning(client_type, client, authorized_client, authorized_admin, project_data, sutra_data, meaning_data,):
    ...
    assert response.status_code == status.HTTP_201_CREATED
    response = authorized_admin.post(f"/isha/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}", json=meaning_data())
    assert response.status_code == status.HTTP_201_CREATED
    ...
    response = test_client.get(f"/isha/sutras/test/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}?lang={meaning_data()["language"]}",)
...
def test_add_meaning(client_type, expected_status, client, authorized_client, authorized_admin, project_data, sutra_data, meaning_data,):
    ...
    clients = {"client": client, "authorized_client": authorized_client, "authorized_admin": authorized_admin,}
    test_client = clients[client_type]
    response = test_client.post(f"/isha/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}", json=meaning_data())
...
def test_update_meaning(client_type, expected_status, client, authorized_client, authorized_admin, project_data, sutra_data, meaning_data,):
    ...
    response = test_client.put(f"/isha/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}?lang={updated_meaning["language"]}", json=updated_meaning)
    assert response.status_code == expected_status
    if response.status_code == status.HTTP_202_ACCEPTED:
        response = test_client.get(f"/isha/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}?lang={meaning_data()["language"]}",)
        assert response.status_code == status.HTTP_200_OK
...
# Map client_type to the corresponding client instance
    clients = {"client": client, "authorized_client": authorized_client, "authorized_admin": authorized_admin,}
    test_client = clients[client_type]
    response = test_client.delete(f"/isha/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}?lang={meaning_data()["language"]}")
    assert response.status_code == expected_status
