...
def test_get_transliteration(client_type, client, authorized_client, authorized_admin, project_data, sutra_data, transliteration_data,):
    ...
    assert response.status_code == status.HTTP_201_CREATED
    response = authorized_admin.post(f"/kena/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}", json=transliteration_data())
    assert response.status_code == status.HTTP_201_CREATED
    ...
    response = test_client.get(f"/kena/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}?lang={transliteration_data()["language"]}",)
...
def test_add_transliteration(client_type, expected_status, client, authorized_client, authorized_admin, project_data, sutra_data, transliteration_data,):
    ...
    clients = {"client": client, "authorized_client": authorized_client, "authorized_admin": authorized_admin,}
    test_client = clients[client_type]
    response = test_client.post(f"/kena/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}", json=transliteration_data())
...
def test_update_transliteration(client_type, expected_status, client, authorized_client, authorized_admin, project_data, sutra_data, transliteration_data,):
    ...
    response = test_client.put(f"/kena/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}?lang={updated_transliteration["language"]}", json=updated_transliteration)
    assert response.status_code == expected_status
...
# Map client_type to the corresponding client instance
    clients = {"client": client, "authorized_client": authorized_client, "authorized_admin": authorized_admin,}
    test_client = clients[client_type]
    response = test_client.delete(f"/kena/sutras/{sutra_data()["project"]["name"]}/{sutra_data()["sutra"]["chapter"]}/{sutra_data()["sutra"]["number"]}/{entity_suffix}?lang={transliteration_data()["language"]}")
    assert response.status_code == expected_status
